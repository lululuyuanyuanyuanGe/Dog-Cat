...
const PdfCard: React.FC<PdfCardProps> = ({ item }) => (
    <div className="h-full w-full relative bg-cream border border-gray-200 p-4 flex flex-col justify-between group-hover:bg-white transition paper-fold shadow-sm hover:shadow-md rotate-[-2deg] hover:rotate-0">
         ...
         <div className="flex-1 flex flex-col gap-4">
             <div className="flex items-start gap-4 mt-2">
                ...
                <div className="min-w-0">
                    <h4 className="font-bold text-slate text-sm truncate leading-tight mb-1" title={item.title}>{item.title}</h4>
                    <span className="text-[10px] font-mono text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded">{item.sizeLabel || 'DOC'}</span>
                </div>
             </div>

             {item.caption && (
                 <div className="bg-yellow-50 border-l-4 border-yellow-200 p-2 text-xs text-slate-600 font-hand italic leading-tight shadow-sm relative">
                     "{item.caption}"
                 </div>
             )}
         </div>
         ...
         <a 
            ...
            className="w-full mt-auto flex items-center justify-center gap-2 py-2 rounded-xl border border-gray-200 text-xs font-bold text-slate/60 hover:bg-coral hover:border-coral hover:text-white transition cursor-pointer"
         >
